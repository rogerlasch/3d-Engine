...
namespace gpp
{
bool collision3d::isColliding(GeometricShape* s1, GeometricShape* s2)
{
if((s1==NULL)||(s2==NULL))
{
return false;
...
{
return sphereBox(((s1->geometricType==GTYPE_SPHERE) ? (sphere3d*)s1 : (sphere3d*)s2), ((s1->geometricType==GTYPE_BOX) ? (box3d*)s1 : (box3d*)s2));
}
case GTYPE_SPHERE|GTYPE_LINE:
return sphereLine(((s1->geometricType==GTYPE_SPHERE) ? (sphere3d*)s1 : (sphere3d*)s2), ((s1->geometricType==GTYPE_LINE) ? (line3d*)s1 : (line3d*)s2));
case GTYPE_BOX:
return boxBox((box3d*)s1, (box3d*)s2);
case GTYPE_BOX|GTYPE_LINE:
return boxLine( ((s1->geometricType==GTYPE_BOX) ? (box3d*)s1 : (box3d*)s2), ((s1->geometricType==GTYPE_LINE) ? (line3d*)s1 : (line3d*)s2));
case GTYPE_LINE:
...
{
float sqdist=vector3d::get_squared_distance(s1->center, s2->center);
...
float sqdist=0;
if(s->center.x<=min.x) sqdist+=((min.x-s->center.x)*(min.x-s->center.x));
else if(s->center.x>max.x) sqdist+=((s->center.x-max.x)*(s->center.x-max.x));

if(s->center.y<=min.y) sqdist+=((min.y-s->center.y)*(min.y-s->center.y));
else if(s->center.y>max.y) sqdist+=((s->center.y-max.y)*(s->center.y-max.y));

if(s->center.z<=min.z) sqdist+=((min.z-s->center.z)*(min.z-s->center.z));
else if(s->center.z>max.z) sqdist+=((s->center.z-max.z)*(s->center.z-max.z));
